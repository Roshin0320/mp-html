...
export function markedAlert() {
  ...
  function renderAlert(token) {
    ...
    text = text.replace(/<p .*?>/g, `<p class="${meta.contentClassName}">`);
    let tmpl = `<div class="blockquote ${meta.className} ${meta.className}-${meta.variant}" class="${meta.wrapperClassName}">\n`;
  ...
  return {
    ...
    extensions: [
      { name: "alert", level: "block", renderer: renderAlert },
      {
        name: "alertContainer",
        level: "block",
        start(src) {
 ...
 */
export function resolveVariants(variants) {
