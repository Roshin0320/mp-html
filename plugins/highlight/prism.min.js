/* PrismJS 1.30.0
https://prismjs.com/download#themes=prism&languages=markup+css+clike+javascript+aspnet+bash+c+csharp+cpp+css-extras+csv+dns-zone-file+docker+editorconfig+git+go+go-module+graphql+http+hpkp+hsts+ignore+java+javadoc+javadoclike+javastacktrace+jq+jsdoc+js-extras+json+json5+jsonp+jsstacktrace+js-templates+less+log+makefile+markdown+markup-templating+mata+mongodb+nginx+php+phpdoc+php-extras+plsql+powershell+python+ruby+rust+sas+sass+scss+scheme+sql+typescript+typoscript+vim+xml-doc+yaml */
